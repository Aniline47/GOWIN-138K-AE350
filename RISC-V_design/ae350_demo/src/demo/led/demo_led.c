...
#define GPIO_LED_USED_MASK_LED		0x7			// GPIO pins output as LED
#define NUM_LED					3			// LED number


// GPIO callback event
void gpio_callback_led(uint32_t event)
{
	switch (event)
	{
...
int demo_led(void)
{
	...
	// Initializes GPIO
	GPIO_Dri->Initialize(gpio_callback_led);

	// Set GPIO direction (GPIO pins as led: output)
	GPIO_Dri->SetDir(GPIO_LED_USED_MASK_LED, AE350_GPIO_DIR_OUTPUT);
