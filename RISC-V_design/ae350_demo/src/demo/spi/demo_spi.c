...
#define MASTER2SLAVE        1						// Send data from master to slave
...
int spi_master_mode (void)
{
	...
	// Initializes SPI
	if(SPI_Dri->Initialize(spi_callback)==0){printf("success1\n");};

	// Power full the SPI peripheral
	if(SPI_Dri->PowerControl(AE350_POWER_FULL)==0){printf("success2\n");};

	// Configure the SPI to master, 8-bit data length and bus speed to 1MHz
	if(SPI_Dri->Control(AE350_SPI_MODE_MASTER |
			 	  	 AE350_SPI_CPOL0_CPHA0 |
					 AE350_SPI_MSB_LSB |
					 AE350_SPI_DATA_BITS(8), 1000000)==0){printf("success3\n");};
	if(SPI_Dri->Control(AE350_SPI_TX_HEADER_LENGTH , 2)==0){printf("success4\n");};
	...
	if(SPI_Dri->Send(data_out, TOTAL_TRANSFER_SIZE)==AE350_DRIVER_ERROR_BUSY){
		printf("error\n");
	}
	}
	wait_complete();
