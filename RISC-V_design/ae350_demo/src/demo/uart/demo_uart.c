 ...
 * Scenario:
 *
 * After initialized, the UART will send out the string "Input 'G' to receive
 * a message" and then wait for receiving data. If the "carriage return" is received
 * (i.e. the 'G' key is pressed on PC side under terminal emulation program),
...
int demo_uart(void)
{
    ...
    UARTdrv->Send("\r\nInput 'G' to receive a message", 34);
    wait_complete();
