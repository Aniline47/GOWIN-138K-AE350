...
// Key: GPIO_5/4/3
#define GPIO_USED_MASK_KEY       0x38    	// Which GPIOs to input

extern AE350_DRIVER_GPIO Driver_GPIO;	// GPIO


// GPIO callback
void gpio_callback_key(uint32_t event)
{
	// Don't do anything.
	switch (event)
	{
...
void setup_gpio(void)
{
	...
	// Initializes GPIO
	GPIO_Dri->Initialize(gpio_callback_key);

	// Set GPIO direction
	GPIO_Dri->SetDir(GPIO_USED_MASK_KEY, AE350_GPIO_DIR_INPUT);		// SW1 ~ SW3 input

	// Set switches interrupt mode to negative edge and enable it
	GPIO_Dri->Control(AE350_GPIO_SET_INTR_NEGATIVE_EDGE | AE350_GPIO_INTR_ENABLE, GPIO_USED_MASK_KEY, 0, 0);
