...
/*
 ********************************************************************************************
 * This demo program shows how to develop digital signal processing systems with DSP
 * library and math library easily and quickly.
 *
 * Scenario:
 *
 * Call DSP library and math library to do Radix-2 complex fast Fourier transform (CFFT) and
 * inverse fast Fourier transform (CIFFT). These functions implement the famous Cooley-Tukey
 * algorithm to transform signals from time domain to frequency domain. The complex data in
 * the input vector are arranged as [real, imaginary, real, imaginary..., real, imaginary].
 *
 * Note: linking DSP library and math library.
 ********************************************************************************************
 */
...
#include <stdlib.h>
#include <math.h>
#include <riscv_dsp_transform_math.h>		// toolchains\nds32le-elf-mculib-v5\riscv32-elf\include
...
static inline float32_t dump2_goldendiff_q15(const char *hdr, float32_t *buf, q15_t *buf2, uint32_t size, uint8_t type)
{
	...
	printf("***** output [scaleingout] | ** [golden] * | **** abs diff ****\r\n");
...
{
	0.0000000000, 0.1950903237, 0.3826834559, 0.5555702448, 0.7071068287, 0.8314695954, 0.9238795638, 0.9807853103,
	1.0000000000, 0.9807852507, 0.9238795042, 0.8314695358, 0.7071067691, 0.5555701852, 0.3826832771, 0.1950903088,
	-0.0000000874, -0.1950904876, -0.3826834559, -0.5555703044, -0.7071068883, -0.8314697742, -0.9238796830, -0.9807852507,
	-1.0000000000, -0.9807852507, -0.9238794446, -0.8314694762, -0.7071065307, -0.5555703044, -0.3826834261, -0.1950902343,
};
...
int demo_dsp(void)
{
	...
	printf("CFFT_RD2 out scale up by %d\r\n", (2 << TRANSFORM_SAMPLES_LOG));
	printf("--------------------------------------------\r\n\r\n");
	...
	printf("CIFFT_RD2 out scale up by %d\r\n", 2);
	printf("--------------------------------------------\r\n");
